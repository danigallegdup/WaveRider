...
data_dir = report_dir / "../GeneratedSongData"
png_output_dir = report_dir / "../pngs"
png_output_dir.mkdir(exist_ok=True)
pitch_classes = ['C', 'C#', 'D', 'D#', 'E', 'F', 'F#', 'G', 'G#', 'A', 'A#', 'B']

# Iterate through all JSON files in the GeneratedSongData directory
for song_data_file in data_dir.glob("*_song_data.json"):
    song_base = song_data_file.stem.replace("_song_data", "")
    ...
    try:
        ...
        print(f"Error loading audio file {song_base}: {e}")
    ...
    plt.savefig(png_output_dir / f"{song_base}.png")
    plt.close()

    print(f"✅ Dashboard saved for {song_base}")